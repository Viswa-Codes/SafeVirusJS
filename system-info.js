...
class CodeFileManager {
  constructor(baseDir = './code-files') {
    ...
    // Supported source code file extensions for discovery
    this.supportedExtensions = ['.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.cpp', '.c', '.h', '.cs', '.php', '.go', '.rs','.env'];
    // Folders to skip during recursive scanning (node_modules, .git, build artifacts, etc.)
    ...
    try {
      ...
      entries.forEach(entry => {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            // Skip folders in the skip list and hidden folders (starting with .)
            if (!this.skipFolders.includes(entry) && !entry.startsWith('.')) {
...
EXAMPLES:
    ...
    node system-info.js sysinfo               # Show system info
